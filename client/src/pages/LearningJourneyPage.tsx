...
export default function LearningJourneyPage() {
  ...
  const getStepProgress = (stepId: number): { completedSections: number, totalSections: number, isCompleted: boolean, canAccess: boolean } => {
    ...
    sections.forEach(section => {
      ...
      if (sectionProgress?.isCompleted) completedSections++; // Fixed: use isCompleted instead of status
    });
    ...
    return {
      ...
      canAccess: stepId === 1 // Step 1 is always accessible, others checked separately
    };
  };
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      case 'Beginner': return 'bg-success/10 text-success border border-success/20';
      case 'Intermediate': return 'bg-warning/10 text-warning border border-warning/20';
      case 'Advanced': return 'bg-destructive/10 text-destructive border border-destructive/20';
